-- Thamilchselvan Kethareswaran	ASSIGNMENT 3 FILTER DATA
...
-- 4
-- A
SELECT first_name, last_name, phone, role FROM employees WHERE NOT (role = 'Manager' OR role = 'Assistant');
-- B
SELECT * FROM stock_items WHERE NOT (item LIKE '%cat%' OR item  LIKE  '%dog%') AND inventory < 20 ;

-- 5
-- A
SELECT first_name, last_name, role, phone FROM employees ORDER BY last_name ASC; 
-- B
